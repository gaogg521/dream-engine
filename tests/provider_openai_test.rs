use dream::provider::LlmProvider;
use dream::provider::compat::ProviderCompat;
use dream::provider::debug::DebugConfig;
use dream::provider::openai::OpenAIProvider;
use dream::types::llm::{LlmEvent, LlmRequest};
use dream::types::message::{ContentBlock, Message, Role, StopReason};
use serde_json::json;
...
async fn test_openai_api_error_non_success_status() {
    ...
    match result.unwrap_err() {
        dream::provider::ProviderError::Api { status, .. } => {
...
async fn test_openai_rate_limited() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        dream::provider::ProviderError::RateLimited { retry_after_ms } => {
