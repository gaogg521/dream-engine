...
./target/release/dream config init
# Edit the generated config (run `dream config path` to find it)

# Single-shot mode
dream "Read Cargo.toml and explain the dependencies"
...
```bash
dream --max-turns 10 "Run the task"
dream --max-tool-call-malformed-turns 2 "Run the task"
dream --max-tool-call-failure-turns 2 "Run the task"
```
