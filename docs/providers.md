...
```bash
dream --profile claude-fast "Quick question"
dream --profile claude-deep "Deep security audit"
dream --profile dev "Create a GitHub issue"
```
...
```bash
dream --profile deepseek-v4-pro --thinking enabled
```
...
3. Credentials are saved alongside the global config (run `dream config path` to find the directory)
