...
command = "echo \"$(date): ${TOOL_INPUT_COMMAND}\" >> .dream/audit.log"
...
```bash
# CLI — logs to /tmp/dream-logs at debug level
dream --log-dir /tmp/dream-logs --log-level debug "Read Cargo.toml"
```
...
| `"dream_engine_providers=debug,info"` | Debug for providers, info for everything else |
...
| Linux | `$XDG_STATE_HOME/dream/logs/` or `~/.local/state/dream/logs/` |
| Windows | `{data_local_dir}/dream/logs/` |
...
```json
{"timestamp":"2026-05-13T12:12:52.431Z","level":"INFO","fields":{"message":"mcp server connected","server":"sentry","tools":20},"target":"dream_engine_mcp","spans":[{"name":"agent_run","session_id":"abc-123","msg_id":"msg-456"}]}
```
...
| Field | Description |
|-------|-------------|
| `target` | Source crate (`dream_engine_agent`, `dream_engine_providers`, `dream_engine_mcp`, etc.) |
...
grep '"session_id":"abc-123"' 2026-05-13.dream.log | jq .
```

### Library Integration

When dream is used as a library (e.g. embedded in a backend server), the `create_file_layer()` API provides a composable tracing layer:

```rust
use dream_engine_config::logging::{ResolvedLogging, create_file_layer};

let resolved = ResolvedLogging {
    enabled: true,
    level: "dream_engine_agent=debug,dream_engine_providers=debug".to_string(),
...
tracing_subscriber::registry()
    .with(your_app_layer)
    .with(layer)  // dream logs → separate dream.log file
    .init();
```

The host application owns the global subscriber; dream library crates only emit tracing events and never initialize a subscriber themselves.
...
1. **Global**: `<config_dir>/dream/AGENTS.md` — user-level instructions for all projects
...
Running dream in `packages/server/` produces a system prompt containing both files, workspace first, then server.
...
```
<config_dir>/dream/projects/<sanitized-project-path>/memory/
...
```bash
export DREAM_MEMORY_DIR=/custom/path
```
...
plan_directory = ".dream/plans"  # Where plan files are saved
```
