...
2. Substitutes variables (`$ARGUMENTS`, `$0`, `${DREAM_SKILL_DIR}`)
...
|----------|------|-------------|
| 1 | `.dream/skills/` | Project-local skills (checked-in with the repo) |
| 2 | `<CONFIG_DIR>/dream/skills/` | User-global skills (see below) |
| 3 | `.dream/commands/` | Legacy flat `.md` files (backward compatibility) |
...
>
> Run `dream skills path` to see the actual paths on your machine.
...
| `${DREAM_SKILL_DIR}` | Absolute path to the directory containing this skill's `SKILL.md` |
...
Working directory: ${DREAM_SKILL_DIR}
```
...
Use `dream skills path` to see which directories are being scanned and whether
they exist on disk:

```
$ dream skills path
User:    ~/Library/Application Support/dream/skills  (exists)
Project: /path/to/repo/.dream/skills                 (exists)
Legacy:  /path/to/repo/.dream/commands                (not found)
```
