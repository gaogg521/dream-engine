# dream JSON Stream Protocol Spec

> This protocol defines the communication between dream (Rust CLI) and a host client (e.g., Dream UI Electron app) via stdin/stdout JSON Lines.
...
- **Encoding**: UTF-8
- **Activation**: `dream --json-stream [other flags]`
...
dream --json-stream --session-id my-conv-123 --provider openai --model gpt-4o

# Resume an existing session
dream --json-stream --resume my-conv-123 --provider openai --model gpt-4o

# Fork an existing session into a new id (read it from `ready.session_id`)
dream --json-stream --resume my-conv-123 --fork-session --provider openai --model gpt-4o
```
