# dream justfile — run tasks with `vx just <recipe>`
...
test-e2e-anthropic:
    @just _run "{{ cargo }} nextest run -p dream-engine-agent --profile e2e --test e2e -E 'test(anthropic)'"

test-e2e-openai:
    @just _run "{{ cargo }} nextest run -p dream-engine-agent --profile e2e --test e2e -E 'test(openai)'"
...
test-acceptance:
    @just _run "{{ cargo }} nextest run -p dream-engine-agent --profile e2e --test acceptance"

test-acceptance-memory:
    @just _run "{{ cargo }} nextest run -p dream-engine-agent --profile e2e --test acceptance -E 'test(memory)'"

test-acceptance-compact:
    @just _run "{{ cargo }} nextest run -p dream-engine-agent --profile e2e --test acceptance -E 'test(compact)'"
...
[unix]
version:
    @{{ cargo }} pkgid -p dream-engine-cli | sed 's/.*#//'

[windows]
version:
    @({{ cargo }} pkgid -p dream-engine-cli) -replace '.*#'
