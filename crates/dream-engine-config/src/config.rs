...
use dream_engine_types::llm::ThinkingConfig;
...
/// `image_input = "supported"`, or injected by an embedding host (Dream UI) that
...
fn resolve_api_key(cli_key: Option<&str>, config_key: Option<&str>, provider: ProviderType) -> anyhow::Result<String> {
    ...
    anyhow::bail!(
        "No API key found. Provide via --api-key, config file, environment variable \
         (API_KEY, ANTHROPIC_API_KEY, or OPENAI_API_KEY), or run 'dream auth login'."
    )
}
...
pub fn app_config_dir() -> Option<PathBuf> {
    dirs::config_dir().map(|d| d.join("dream"))
}
...
pub fn global_config_path() -> PathBuf {
    app_config_dir()
        .unwrap_or_else(|| PathBuf::from("dream"))
        .join("config.toml")
}

fn project_config_path() -> PathBuf {
    PathBuf::from(".dream.toml")
}

fn load_config_file(path: &Path) -> ConfigFile {
    match std::fs::read_to_string(path) {
        Ok(content) => toml::from_str(&content).unwrap_or_else(|e| {
            tracing::warn!(target: "dream_engine_config", path = %path.display(), error = %e, "failed to parse config file");
            ConfigFile::default()
        }),
...
const DEFAULT_CONFIG_TEMPLATE: &str = r#"# dream configuration
...
# OAuth settings (for `dream auth login` with Claude.ai account)
# [auth]
...
[session]
enabled = true
directory = ".dream/sessions"  # relative to project root
...
# dir = "~/Library/Logs/dream"    # log directory (default: platform-specific)
