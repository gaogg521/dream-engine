...
const MEMORY_DIR_ENV: &str = "DREAM_MEMORY_DIR";
...
///
/// Resolution order:
///   1. `DREAM_MEMORY_DIR` environment variable (explicit override)
///   2. `app_config_dir()` from `dream-config` (platform-aware default)
///
...
pub fn memory_base_dir() -> Option<PathBuf> {
    ...
    dream_engine_config::config::app_config_dir()
}
