...
async fn load_skill_file(
    ...
    // used for ${DREAM_SKILL_DIR} variable substitution in skill content.
...
///
/// Examples:
/// - base=`<config_dir>/dream/skills`, target=`<config_dir>/dream/skills/db/migrate` → `"db:migrate"`
/// - base=`<config_dir>/dream/skills`, target=`<config_dir>/dream/skills/my-skill` → `"my-skill"`
