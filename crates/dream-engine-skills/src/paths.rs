...
use dream_engine_config::config::app_config_dir;

// ---------------------------------------------------------------------------
// User-level directories (<config_dir>/dream/)
// ---------------------------------------------------------------------------

/// Return the user-level skills directory: `<config_dir>/dream/skills/`
///
...
/// Return the user-level legacy commands directory: `<config_dir>/dream/commands/`
...
/// Find all project-level `.dream/skills/` directories by walking up from
...
/// Find all project-level `.dream/commands/` directories (legacy), same walk.
...
/// `.dream/<subdir>/` directories that exist. Returns deepest-first.
fn walk_up_dirs(cwd: &Path, subdir: &str) -> Vec<PathBuf> {
    ...
    loop {
        let candidate = current.join(".dream").join(subdir);
        if candidate.is_dir() {
            dirs.push(candidate);
        }
