...
mod tests {
    ...
    #[test]
    fn test_dream_skill_dir_substitution() {
        let result = substitute_arguments("dir=${DREAM_SKILL_DIR}", None, &[], Some("/my/skill"), None);
        assert_eq!(result, "dir=/my/skill");
    }

    #[test]
    fn test_dream_session_id_substitution() {
        let result = substitute_arguments("sid=${DREAM_SESSION_ID}", None, &[], None, Some("sess-123"));
...
mod supplemental_tests {
    ...
    // -----------------------------------------------------------------------
    // TC-6.x: ${DREAM_SKILL_DIR} substitution
    // -----------------------------------------------------------------------
    ...
    fn tc_6_1_skill_dir_replaced() {
        let r = substitute_arguments(
            ...
            None,
            &[],
            Some("/home/user/.dream/skills/my-skill"),
            None,
        );
        assert_eq!(r, "cd /home/user/.dream/skills/my-skill");
    }

    #[test]
    fn tc_6_2_skill_dir_none_not_replaced() {
        // skill_root = None → ${DREAM_SKILL_DIR} stays unreplaced
        let r = substitute_arguments("cd ${DREAM_SKILL_DIR}", None, &[], None, None);
        assert_eq!(r, "cd ${DREAM_SKILL_DIR}");
    }

    #[test]
    fn tc_6_3_skill_dir_multiple_occurrences() {
        let r = substitute_arguments(
            "${DREAM_SKILL_DIR}/a and ${DREAM_SKILL_DIR}/b",
            None,
            &[],
            Some("/skills/foo"),
            None,
    ...
    // -----------------------------------------------------------------------
    // TC-7.x: ${DREAM_SESSION_ID} substitution
    // -----------------------------------------------------------------------

    #[test]
    fn tc_7_1_session_id_replaced() {
        let r = substitute_arguments("Session: ${DREAM_SESSION_ID}", None, &[], None, Some("abc-123"));
        assert_eq!(r, "Session: abc-123");
    }

    #[test]
    fn tc_7_2_session_id_none_not_replaced() {
        let r = substitute_arguments("Session: ${DREAM_SESSION_ID}", None, &[], None, None);
        assert_eq!(r, "Session: ${DREAM_SESSION_ID}");
    }
    ...
    fn tc_9_1_multiple_placeholder_types() {
        let r = substitute_arguments(
            "cd ${DREAM_SKILL_DIR} && run $ARGUMENTS[0] with $ARGUMENTS",
            Some("alpha beta"),
            &[],
            Some("/skills/foo"),
            None,
