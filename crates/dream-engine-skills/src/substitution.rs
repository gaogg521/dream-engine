...
/// 5. Skill directory: `${DREAM_SKILL_DIR}` → `skill_root`
/// 6. Session ID: `${DREAM_SESSION_ID}` → `session_id`
...
pub fn substitute_arguments(
    ...
    if let Some(root) = skill_root {
        result = result.replace("${DREAM_SKILL_DIR}", root);
    }

    // 6. ${DREAM_SESSION_ID}
    if let Some(sid) = session_id {
        result = result.replace("${DREAM_SESSION_ID}", sid);
    }
