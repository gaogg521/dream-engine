...
use dream_engine_config::compat::{OpenAiApiMode, ProviderCompat};
use dream_engine_providers::LlmProvider;
use dream_engine_providers::openai::OpenAIProvider;
use dream_engine_types::llm::{LlmEvent, LlmRequest};
use dream_engine_types::message::{ContentBlock, Message, Role, StopReason};
...
async fn test_openai_api_error_non_success_status() {
    ...
    match result.unwrap_err() {
        dream_engine_providers::ProviderError::Api { status, .. } => {
...
async fn test_aio_140_openai_tools_wire_shape_mismatch_error_is_readable_and_not_retried() {
    ...
    match result.unwrap_err() {
        dream_engine_providers::ProviderError::Api { status, message } => {
...
async fn test_openai_rate_limited() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        dream_engine_providers::ProviderError::RateLimited { retry_after_ms, body } => {
