...
use dream_engine_config::shell::{ResolvedShell, default_shell, render_shell_prompt};
use dream_engine_memory::prompt::build_memory_prompt_minimal;
use dream_engine_skills::prompt::format_skills_within_budget;
...
pub(crate) fn build_system_prompt_with_shell_and_tool_policy(
    ...
    if toon_enabled {
        let toon_section = cache
            .sections
            .entry("toon")
            .or_insert_with(|| dream_engine_compact::toon_format_instructions().to_string());
