...
mod tests {
    ...
    impl Tool for MockDeferredTool {
        ...
        async fn execute(&self, input: serde_json::Value) -> dream_engine_types::tool::ToolResult {
            if input.get("tasks").is_none() {
                return dream_engine_types::tool::ToolResult {
        ...
        fn category(&self) -> dream_engine_protocol::events::ToolCategory {
            dream_engine_protocol::events::ToolCategory::Exec
        }
    }
