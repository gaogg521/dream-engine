...
use dream_engine_protocol::events::{OutputType, ProtocolEvent, ToolCategory, ToolInfo, ToolStatus};
...
use dream_engine_types::skill_types::ContextModifier;
use dream_engine_types::tool::ToolResult;

use dream_engine_tools::{registry::ToolRegistry, truncate_utf8};
...
pub async fn execute_tool_calls(
    ...
    compaction_level: dream_engine_compact::CompactLevel,
    toon_enabled: bool,
...
async fn execute_single(
    ...
    let (result, modifier, follow_up_blocks) = match registry.get(name) {
        Some(tool) => {
            ...
            let content = dream_engine_compact::compact_output(&error_content, compaction_level);
            let content = if toon_enabled {
                dream_engine_compact::compact_output_toon(&content)
            } else {
                content
            };
    ...
    if let Some(hook_engine) = hooks {
        ...
        for msg in messages {
            tracing::info!(target: "dream_engine_agent", hook_message = %msg, "post-tool-use hook output");
        }
    }

    let duration_ms = start.elapsed().as_millis() as u64;
    tracing::info!(target: "dream_engine_agent", duration_ms, success = !result.is_error, "tool execution completed");
