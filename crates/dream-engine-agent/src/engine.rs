...
pub struct AgentEngine {
    ...
    msg_id: String,
    /// Turn id for the NEXT run, supplied by the host (e.g. Dream Core passes
...
impl AgentEngine {
    ...
    /// (e.g. Dream Core) pass them in so the session-side anchor matches their
    ...
    async fn run_inner(&mut self, content_blocks: Vec<ContentBlock>, msg_id: &str) -> Result<AgentResult, AgentError> {
        ...
        loop {
            if let Some(limit) = guards.turn_budget_reached() {
                ...
                warn!(target: "dream_engine_agent", limit, "stopping agent run at turn budget");
    ...
    fn record_turn_usage(&mut self, turn_usage: &TokenUsage) -> bool {
        ...
        if context_usage == 0 {
            debug!(target: "dream_engine_agent", "provider omitted turn usage; retaining local context projection");
            return false;
        }
...
impl AgentEngine {
    ...
    pub fn init_session(&mut self, provider_name: &str, cwd: &str, session_id: Option<&str>) -> AnyhowResult<()> {
        if let Some(mgr) = &self.session_manager {
            ...
            mgr.save(&session)?;
            info!(target: "dream_engine_agent", session_id = %session.id, provider = %provider_name, model = %self.model, "session started");
    ...
    pub async fn run_stop_hooks(&self) {
        if let Some(hook_engine) = &self.hooks {
            ...
            for msg in messages {
                info!(target: "dream_engine_agent", hook_message = %msg, "stop hook output");
            }
        }
    }
}
