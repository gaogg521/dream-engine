...
mod tests_set_config {
    ...
    use dream_engine_config::compat::ReasoningCompat;
    ...
    fn make_engine(model: &str) -> super::AgentEngine {
        super::AgentEngine {
            ...
            compact_config: dream_engine_config::compact::CompactConfig::default(),
    ...
    fn set_config_enables_thinking() {
        ...
        match &engine.thinking {
            Some(dream_engine_types::llm::ThinkingConfig::Enabled { budget_tokens }) => {
    ...
    fn set_config_disables_thinking() {
        let mut engine = make_engine("m");
        engine.thinking = Some(dream_engine_types::llm::ThinkingConfig::Enabled { budget_tokens: 8000 });
        let changes = engine.apply_config_update(None, None, Some("disabled".into()), None, None, None);
        match &engine.thinking {
            Some(dream_engine_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_thinking_budget_only_updates_existing_enabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(dream_engine_types::llm::ThinkingConfig::Enabled { budget_tokens: 5000 });
    ...
    fn set_config_thinking_budget_ignored_when_disabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(dream_engine_types::llm::ThinkingConfig::Disabled);
        let changes = engine.apply_config_update(None, None, None, Some(20000), None, None);
        match &engine.thinking {
            Some(dream_engine_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_thinking_applies_when_capability_is_false() {
        ...
        assert!(matches!(
            engine.thinking,
            Some(dream_engine_types::llm::ThinkingConfig::Enabled { budget_tokens: 10000 })
        ));
    }
...
mod tests_phase6 {
    ...
    use dream_engine_tools::registry::ToolRegistry;
    use dream_engine_types::llm::{LlmEvent, LlmRequest};
    use dream_engine_types::skill_types::{ContextModifier, EffortLevel};
...
mod tests_plan_mode {
    ...
    use dream_engine_tools::registry::ToolRegistry;
    use dream_engine_types::llm::{LlmEvent, LlmRequest};
    use dream_engine_types::skill_types::{ContextModifier, PlanModeTransition};
...
mod tests_handle_command {
    ...
    #[tokio::test]
    async fn run_treats_dream_files_marker_as_plain_text() {
        let mut engine = make_engine_with_provider(Arc::new(SingleResponseProvider));
        let input = "discuss [[DREAM_FILES]] and file.txt";
...
mod tests_tool_policy_enforcement {
    ...
    fn build_request_advertises_read_image_to_models_without_image_input() {
        use dream_engine_tools::read_image::ReadImageTool;
