...
use dream_engine_agent::compact::auto::{BOUNDARY_PREFIX, autocompact, should_autocompact};
use dream_engine_agent::compact::emergency::is_at_emergency_limit;
use dream_engine_agent::compact::micro::{CLEARED_TOOL_RESULT, microcompact};
use dream_engine_agent::compact::state::CompactState;
...
async fn autocompact_triggers_llm_summary() {
    ...
    let config = {
        ...
        dream_engine_config::config::Config {
    ...
    let provider = dream_engine_providers::create_provider(&config);
