...
use dream_engine_agent::confirm::ToolConfirmer;
...
use dream_engine_config::config::{Config, ProviderType, SessionConfig, ToolsConfig};
...
fn openai_config(api_key: &str) -> Config {
    Config {
        ...
        tools: ToolsConfig {
            auto_approve: true,
            allow_list: vec![],
            skills: dream_engine_config::config::SkillsPermissionConfig::default(),
        },
...
impl dream_engine_tools::Tool for FixedOutputTool {
    fn name(&self) -> &str {
        &self.name
    }
    ...
    fn category(&self) -> dream_engine_protocol::events::ToolCategory {
        dream_engine_protocol::events::ToolCategory::Info
    }
    ...
    async fn execute(&self, _input: serde_json::Value) -> dream_engine_types::tool::ToolResult {
        dream_engine_types::tool::ToolResult {
