...
//! `.dream/skills/` and `.dream/commands/` layout, then exercises the full
...
use dream_engine_agent::context::{SystemPromptCache, build_system_prompt};
use dream_engine_agent::skill_tool::SkillTool;
use dream_engine_agent::skills::loader::load_all_skills;
use dream_engine_agent::skills::permissions::SkillPermissionChecker;
use dream_engine_agent::skills::types::SkillMetadata;
...
/// Create a project-like temp directory with `.git` marker and `.dream/skills/` + `.dream/commands/`.
...
fn make_project() -> (TempDir, PathBuf) {
    ...
    // Skills directory
    let skills_dir = root.join(".dream").join("skills");
    fs::create_dir_all(&skills_dir).unwrap();

    // Commands directory (legacy)
    let commands_dir = root.join(".dream").join("commands");
...
async fn e2_legacy_commands_discovered() {
    ...
    println!("E2 PASS: legacy command 'legacy-cmd' discovered from .dream/commands/");
}
...
// ---------------------------------------------------------------------------
// E8: Full SkillTool execution (db:migrate with $ARGUMENTS + ${DREAM_SKILL_DIR})
// ---------------------------------------------------------------------------
...
async fn e8_full_execution() {
    ...
    assert!(
        !result.content.contains("${DREAM_SKILL_DIR}"),
        "E8 FAIL: ${{DREAM_SKILL_DIR}} not expanded. Got: {}",
        result.content
    );
    println!("E8 PASS: full execution with $ARGUMENTS and ${{DREAM_SKILL_DIR}} substitution");
}
