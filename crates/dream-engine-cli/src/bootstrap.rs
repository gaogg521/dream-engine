//! Shared initialization steps for dream-cli entry points.
//!
...
//!
//! Note: `dream_engine_agent` also exposes a module named `bootstrap`
//! (`dream_engine_agent::bootstrap::AgentBootstrap`). Call sites should keep using
//! that fully-qualified path for the engine builder itself; this module is
//! `crate::bootstrap` and only wraps it.

use std::sync::Arc;

use dream_engine_agent::bootstrap::{AgentBootstrap, BootstrapResult};
use dream_engine_agent::output::OutputSink;
use dream_engine_agent::session::{Session, SessionManager};
use dream_engine_compact::CompactLevel;
use dream_engine_config::config::{CliArgs, Config};
use dream_engine_config::logging::{LoggingGuard, create_file_layer};
