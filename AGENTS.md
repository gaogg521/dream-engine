...
Rules and conventions for AI assistants and contributors working on dream.

## Overview

dream is a **multi-provider AI agent CLI** written in Rust. It connects to
...
protocol for host integration (e.g. Electron-based Dream UI).
...
|-------|-------|----------------|
| Bottom | `dream-engine-types` | Shared provider-neutral data types (LLM, message, tool) — zero internal deps |
| Bottom | `dream-engine-compact` | Context compression algorithms (folding, sanitization, tokenization) |
| Mid | `dream-engine-config` | Configuration, ProviderCompat, auth, hooks, logging (`create_file_layer`), **cross-platform shell helpers** |
| Mid | `dream-engine-protocol` | JSON stream protocol (events, commands, approval manager) for host integration |
| Mid | `dream-engine-providers` | LLM provider implementations (Anthropic, OpenAI, Bedrock, Vertex) |
| Mid | `dream-engine-tools` | Local tools (Read, Write, Edit, ExecCommand, Grep, Glob, ViewImage, ToolSearch) |
| Mid | `dream-engine-mcp` | MCP (Model Context Protocol) client |
| Mid | `dream-engine-skills` | Skills system (prompt snippets, hooks, permissions, shell expansion) |
| Mid | `dream-engine-memory` | Long-term cross-session memory (user prefs, feedback, project context) |
| Top | `dream-engine-agent` | Agent engine, session management, orchestration, and agent-level tools such as Spawn |
| Top | `dream-engine-cli` | CLI binary entry point |
...
- DreamCLI is the built-in SDK/library integration path. Do not model it as an
  external agent subprocess when changing Dream host integration code.
...
- All shell invocations must go through `dream_engine_config::shell` module
...
| [json-stream-protocol.md](docs/json-stream-protocol.md) | JSON Lines protocol spec for host integration (e.g. Dream UI) |
